...
Parameter đại diện cho một dị điểm trong một Test Group của một Experiment.
Trong một Test Group có thể có một hoặc nhiều Parameter, các parameter đấy đại diện cho đặc điểm biến thể
của Test Group đấy.
Mỗi paramater có hai trường dữ liệu là Name và Value.

\begin{itemize}
	\bfitem{Name}{là tên của Paramater}
	\bfitem{Value}{là giá trị của Parameter}
\end{itemize}

\begin{figure}[H]
	\centering
	\includegraphics[width = 1\textwidth]{parameter}
	\caption{Parameter}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[width = 0.7\textwidth]{redis-features}
...
\begin{itemize}
	\bfitem{Strings}{dữ liệu văn bản hoặc dữ liệu nhị phân}
	\bfitem{Lists}{tập hợp các chuỗi theo thứ tự thêm vào}
	\bfitem{Sets}{tập hợp các chuỗi không có thứ tự, riêng biệt theo key}
	\bfitem{Sorted Sets}{Các chuỗi được sắp xếp theo một giá trị}
...
\subsubsection{Command phổ biến}

Sau đây là một số command phổ biến được dùng trong Redis:

\begin{itemize}
\bfitem {GET}{Gán giá trị cho một trường}
\bfitem {SET}{Lấy giá trị của một trường}
\bfitem {SADD}{Thêm một giá trị vào một trường tổng hợp}
\bfitem {SMEMBER}{Lấy toàn bộ giá trị trong một trường tổng hợp}
\bfitem {INCR}{Tăng giá trị số học của một trường}
\end{itemize}
