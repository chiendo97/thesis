...
export default function IndexPage({
  ...
  const form = useForm({
    initialValues: { ...experiment, layer_id: Number(layerId as String) } as APIExperiment,
    validate: {},
  });
  ...
  const onSubmit = async (data: APIExperiment) => {
    ...
    await fetch('http://localhost:8080/api/v1/experiment/update', {
      method: 'POST',
      body: JSON.stringify({
        ...data,
    ...
    <Container size="xs">
      ...
      <form onSubmit={form.onSubmit(onSubmit)}>
        ...
        <NumberInput
          ...
          placeholder="Traffic"
          label="Traffic"
          description={`From 0 to ${Math.max(layer.unused_traffic, experiment.traffic)}%`}
          required
          max={Math.max(layer.unused_traffic, experiment.traffic)}
          min={0}
          step={5}
        ...
        <DateRangePicker
          ...
          required
          value={[
            moment(form.values.start_time * 1000).toDate(),
            moment(form.values.end_time * 1000).toDate(),
          ]}
          onChange={(ranges) => {
            if (ranges[0]) {
